...
async function connectToWA() {
    console.log("[🔰] Arslan-XMD Connecting to WhatsApp ⏳️...");
    ...
    conn.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            console.log('[🔰] Arslan-XMD connected to WhatsApp ✅');
...
‎
‎   *_⭐ˢᵗᵃʳ ᵗʰᵉ ʳᵉᵖᵒ_*
‎🔗https://github.com/Arslan-MD/Arslan-XMD
‎╰╼╼╼╼╼╼╼╼╼╼╼╼╼╼╼╼╼╼╯`;
                    
                    await conn.sendMessage(conn.user.id, { 
                        image: { url: `https://files.catbox.moe/x5csyw.jpg` }, 
                        caption: upMessage 
                    });
...
BotActivityFilter(conn);	
 ...
 /// READ STATUS       
  ...
  const udp = botNumber.split('@')[0];
    const immumd = ('923237045919', '923237045919');
...
if (isBanned) return; // Ignore banned users completely
 ...
 /* 
  app.get("/", (req, res) => {
  res.send("Arslan-XMD STARTED ✅");
  });
*/
  app.use(express.static(path.join(__dirname, 'lib')));

app.get('/', (req, res) => {
  res.redirect('/arslan.html');
});
